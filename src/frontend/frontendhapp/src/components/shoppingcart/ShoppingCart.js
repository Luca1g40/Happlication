    ...
    return (shoppingCart.length > 0) ? (
        <div className={"menu-container"}>
            ...
            {Array.from(getOccurrenceProducts(shoppingCart).keys()).map((item, index) =>{
                return (
                    <ShoppingCartItem key={item.id} productName={item.name} productId={item.id} amount={Array.from(getOccurrenceProducts(shoppingCart).values())[index]} updateShoppingCart={shoppingcart => setShoppingCart(shoppingcart)}/>
                );
            })}
                <SubmitButton className={"submit-button button"} buttonText={"Order"} tableId={577} action={Actions.PLACE_ORDER} emptyShoppingcart={()=>setShoppingCart([])} triggerPopUp={()=>setButtonPopup(true)}/>
        </div>
