        ...
        if (!(params.id === undefined)) {
            getProduct(params.id)
                .then(res => {
                    ...
                    setToegevoegdeIngredienten(res.ingredientList.map((ingredient) => {
    ...
    return (params.id === undefined) ? (
        <>
            ...
            <div className={"navigation-buttons"}>
                ...
                <Link to="/staff" className="button products-navigation" onClick={() => {
                    clearData()
                }}>Log out</Link>
            </div>

            <h1>Create product</h1>
            <ProductForm allCategories={allCategories} product={product} creating={true}
                         setSelectedImage={selectedImage => setSelectedImage(selectedImage)}
                         toegevoegdeIngredienten={toegevoegdeIngredienten} disabled={false}
                         handleChange={event => handleChange(event)}
                         removeFromIngredientsList={(target => removeFromIngredientsList(target))}
                         setAddedIngredients={(ingredient) => setToegevoegdeIngredienten(ingredient)}
                         addIngredient={ingredient => addIngredient(ingredient)} errorMeldingText={errorMeldingText}/>
            <div className={"create-button"}>
                <SubmitButton selectedImage={selectedImage} className={"submit-button button"}
                              action={Actions.CREATE_PRODUCT} buttonText={"Create product"}
                              setProduct={product => setProduct(product)} product={product}
                              ingredientList={toegevoegdeIngredienten}
                              setFoutMelding={error => setErrorMeldingText(error)}/>
            </div>
        </>
    ) : (
        <>
            ...
            <h1>Edit product</h1>
            <ProductForm allCategories={allCategories} creating={false}
                         setSelectedImage={selectedImage => setSelectedImage(selectedImage)} product={product}
                         toegevoegdeIngredienten={toegevoegdeIngredienten} disabled={disabled}
                         handleChange={event => handleChange(event)}
                         removeFromIngredientsList={(target => removeFromIngredientsList(target))}
                         setToegevoegdeIngredienten={(ingredient) => setToegevoegdeIngredienten(ingredient)}
                         addIngredient={ingredient => addIngredient(ingredient)} errorMeldingText={errorMeldingText}/>
            <div className={"edit-buttons"}>
                <SubmitButton className={"submit-button button"}
                              setFoutMelding={foutMelding => setErrorMeldingText(foutMelding)} disabled={disabled}
                              setDisabled={disabled => setDisabled(disabled)} action={Actions.UPDATE_PRODUCT}
                              buttonText={"Update"} product={product} ingredientList={toegevoegdeIngredienten}
                              selectedImage={selectedImage}/>
                <div className={"left-buttons"}>
                    <button className={"edit-button button"} onClick={() => setDisabled(false)}
                            disabled={!disabled}>Edit
                    </button>
                    <button className={"cancel-button button"} onClick={() => setDisabled(true)}
