    ...
    return (props.product === undefined) ? (
        <>
            <div className={"crud-form"}>
                ...
                <div>
                    ...
                    <input className={"right-column"} name={"name"} placeholder={""}
                           onChange={(event) => props.handleChange(event)}/>
                </div>

                <div>
                    <label className={"left-column"} htmlFor="productType">Category:</label>
                    <select className={"right-column"} name={"productType"}
                            onChange={(event) => props.handleChange(event)}>
                ...
                <select className={"right-column"} name={"productCategoryName"}
                        onChange={(event) => props.handleChange(event)}>
                ...
                <input className={"right-column"} type={"number"} name={"price"} min={0}
                       onChange={(event) => props.handleChange(event)}/>


                <div className={"add-ingredient-div"}>
                    <label className={"left-column"} htmlFor="product-ingredients-dropdown">Add ingredients</label>
                    <select ref={ref} className={"left-column ingredient-dropdown"}
                            name={"product-ingredients-dropdown"}>
                    ...
                    <button className={"add-ingredient-button"}
                            onClick={() => props.addIngredient(ref.current.value)}>Add ingredient:
                    </button>
                </div>
                ...
                <div className={"ingredient-filter left-column"}>
                    {props.toegevoegdeIngredienten.map((ingredient, i) => {
                        return <button name={"ingredient"} key={i} className={"remove-ingredient-button"}
                                       onClick={() => props.removeFromIngredientsList(ingredient)}>{ingredient} X</button>
                    })}
                </div>
                ...
                <textarea className={"right-column details-text"} name={"details"} placeholder={"enter text..."}
        ...
        <>
            ...
            <div className={"crud-form"}>
                ...
                <label className={"left-column"} htmlFor="name">Name:</label>
                <input className={"right-column"} name={"name"} placeholder={""} disabled={props.disabled}
                       value={props.product.name} onChange={(event) => props.handleChange(event)}/>
                ...
                <label className={"left-column"} htmlFor="productCategoryName">Sub category:</label>
                <select className={"right-column"} name={"productCategoryName"}
                        value={props.product.productCategoryName} disabled={props.disabled}
                        onChange={(event) => props.handleChange(event)}>
                    {props.allCategories.map(category => {
                        return <option key={category.id.id} value={category.name}> {category.name}</option>
                    })}
                ...
                <label className={"left-column"} htmlFor="price">Prize:</label>
                <input className={"right-column"} type={"number"} name={"price"} min={0} disabled={props.disabled}
                       value={props.product.price} onChange={(event) => props.handleChange(event)}/>


                <div className={"add-ingredient-div"}>
                    <label className={"left-column"} htmlFor="product-ingredients-dropdown">Add ingredients</label>
                    <select ref={ref} className={"left-column ingredient-dropdown"}
                            name={"product-ingredients-dropdown"} disabled={props.disabled}>
                        {ingredientList.map(ingredient => {
                            return <option key={ingredient.id} value={ingredient.name}> {ingredient.name}</option>
                        })
                        }
                    </select>
                    <button className={"add-ingredient-button"} onClick={() => props.addIngredient(ref.current.value)}
                            disabled={props.disabled}>Add ingredient:
                    </button>
                </div>
                ...
                <label className={"left-column"} htmlFor="details">Product details:</label>
                <textarea className={"right-column details-text"} name={"details"} placeholder={"enter text..."}
                          value={props.product.details} disabled={props.disabled} onChange={props.handleChange}/>

                <form>
                    <div>
                        <label className={"left-column"} htmlFor="imageFile">Image:</label>
                        <input className={"right-column"} type={"file"} name={"imageFile"}
